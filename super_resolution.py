...
def optimize_latent_codes(args):
	...
	generated_img_resized = tf.image.resize_images(
		generated_img_resized_to_original, tuple(args.perceptual_img_size), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR
	)

	generated_img_for_display = tf.saturate_cast(generated_img_resized_to_original, tf.uint8)

	perceptual_model = PerceptualModel(img_size=(args.input_img_size[0], args.input_img_size[1]))
...
if __name__ == '__main__':
	...
	parser.add_argument('--perceptual-img-size', type=int, nargs=2, default=(128, 128))
